...
const FRONTEND_ORIGIN = process.env.FRONTEND_ORIGIN || process.env.VITE_API_BASE_URL || 'http://localhost:5173';
...
function adminAuth(req, res, next) {
  ...
  try {
    ...
    if (user !== adminUser || pass !== adminPass) return res.status(403).json({ ok: false, error: 'Forbidden' });
    return next();
  } catch (err) {
    return res.status(401).json({ ok: false, error: 'Unauthorized' });
  }
...
function validateCheckoutPayload(body) {
  ...
  for (let i = 0; i < body.items.length; i++) {
    ...
    if (!it || typeof it.name !== 'string' || it.name.trim().length === 0) return `Invalid item at index ${i}: missing name`;
    const qty = Number(it.quantity || it.qty || 1);
    if (!Number.isFinite(qty) || qty < 1 || qty > 999) return `Invalid quantity for item ${it.name}`;
    ...
    if (it.customization && typeof it.customization === 'object') {
      if (it.customization.composite && typeof it.customization.composite === 'string') {
        ...
        if (it.customization.composite.length > 3_000_000) return `Composite image too large for item ${it.name}`;
        if (!it.customization.composite.startsWith('data:image/')) return `Invalid composite image format for item ${it.name}`;
      }
    }
  }
  return null;
...
app.post('/api/payment/create-checkout', async (req, res) => {
  try {
    ...
    const orderRecord = {
      ...
      items: items.map((it) => ({ name: it.name, quantity: it.quantity, price: it.amount || it.price })),
...
app.post('/api/admin/login', (req, res) => {
  try {
    console.log('POST /api/admin/login body:', req.body && typeof req.body === 'object' ? JSON.stringify(req.body) : req.body);
...
app.get('/api/admin/dashboard', verifyJwt, (req, res) => {
  try {
    ...
    let uploadsCount = 0;
    try {
      uploadsCount = fs.existsSync(UPLOAD_DIR) ? fs.readdirSync(UPLOAD_DIR).filter((f) => /\.(png|jpe?g|svg|gif|psd|ai)$/i.test(f)).length : 0;
    } catch (err) {
      uploadsCount = 0;
    }
    ...
    const idxByKey = revenueSeries.reduce((m, item, idx) => { m[item.label] = idx; return m; }, {});
    current.forEach((o) => {
    ...
    const recentOrders = current.slice(0, 10).map((o) => {
      ...
      return {
        ...
        customer,
        items: Array.isArray(o.items) ? o.items : (o.raw && Array.isArray(o.raw.items) ? o.raw.items : []),
        status: o.status || null,
        total,
        previewPaths: Array.isArray(o.previewPaths) ? o.previewPaths.map((p) => (typeof p === 'string' && p.startsWith('http') ? p : origin + p)) : [],
      };
    });

    res.json({ ok: true, stats: { totalRevenue, pendingCount, designsUploaded, completedShipments }, recentOrders, revenueSeries });
  } catch (err) {
...
app.get('/api/admin/orders', verifyJwt, (req, res) => {
  try {
    ...
    pageItems.forEach((o) => {
      if (Array.isArray(o.previewPaths)) {
        o.previewPaths = o.previewPaths.map((p) => (typeof p === 'string' && p.startsWith('http') ? p : origin + p));
      }
      if (o.raw && Array.isArray(o.raw.items)) {
        o.raw.items.forEach((it) => {
          if (it.customization && it.customization.composite_saved && !it.customization.composite_saved.startsWith('http')) {
...
app.get('/api/admin/orders/:id', verifyJwt, (req, res) => {
  try {
    ...
    if (Array.isArray(order.previewPaths)) {
      order.previewPaths = order.previewPaths.map((p) => (typeof p === 'string' && p.startsWith('http') ? p : origin + p));
    }
