...
export type ViewDef = { id: string; label: string; mockupUrlsByColor?: Record<string,string>; mockupUrl?: string; printArea?: Partial<PrintArea>; };
...
export function useConfiguratorState(initialProduct?: any) {
  ...
  const [activeViewId, setActiveViewId] = useState<string | null>(() => (initialProduct?.views?.[0]?.id || initialProduct?.views?.[0]?.name || null));
  ...
  const [selectedSize, setSelectedSize] = useState<string | null>((initialProduct?.sizes && initialProduct.sizes[0]) || null);
  const [quantity, setQuantity] = useState<number>(1);

  const views: ViewDef[] = useMemo(() => (product?.views || []).map((v:any)=>({ id: v.id || v._id || v.name, label: v.label || v.name || 'View', mockupUrlsByColor: v.mockupUrlsByColor || v.mockupByColor || {}, mockupUrl: v.mockupUrl || v.url || v.image, printArea: v.printArea })) , [product]);
  ...
  const popUndo = useCallback((viewId: string) => {
  ...
  const popRedo = useCallback((viewId: string) => {
  ...
  const setProductSafe = useCallback((p:any) => {
    setProduct(p);
    if (p?.views && p.views.length && !activeViewId) setActiveViewId(p.views[0].id || p.views[0].name || p.views[0]._id);
    setSelectedVariant(p?.variants?.[0] || null);
  }, [activeViewId]);

  const viewsWithArtworkCount = useMemo(() => Object.keys(viewStates).filter(k=>!!(viewStates[k])).length, [viewStates]);

  const totalPrice = useMemo(() => basePrice + (viewsWithArtworkCount * printSideFee), [basePrice, viewsWithArtworkCount, printSideFee]);
