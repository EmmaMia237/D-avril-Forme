...
const CanvasStage = forwardRef(function CanvasStage(
  ...
  useEffect(() => {
    ...
    function keepInside(obj:any){
      ...
      if (bound.left + bound.width > w) obj.left = Math.min(w - bound.width, obj.left - (bound.left + bound.width - w));
      if (bound.top + bound.height > h) obj.top = Math.min(h - bound.height, obj.top - (bound.top + bound.height - h));
    }
  ...
  useImperativeHandle(ref, () => ({
    loadViewJson: (json) => {
      ...
      try {
        c.clear();
        if (json) c.loadFromJSON(json, ()=>{ c.renderAll(); lastSavedJsonRef.current = JSON.stringify(c.toJSON()); });
        else { c.renderAll(); lastSavedJsonRef.current = null; }
    ...
    getViewJson: () => {
      ...
      try { return JSON.stringify(canvasRef.current.toJSON()); } catch (e) { return null; }
    },
    ...
    addImageFromDataUrl: async (dataUrl) => {
      ...
      if (dataUrl.trim().startsWith('<svg')) {
        ...
        return new Promise<void>((res)=>{
          FabricImage.fromURL(dataUrl, (img:any)=>{
    ...
    addText: (text, opts={}) => {
      if (!canvasRef.current) return;
      const c = canvasRef.current;
      const it = new IText(text || 'Text', { left: c.getWidth()/2, top: c.getHeight()/2, originX:'center', originY:'center', fontSize: 36, fill: '#000', ...opts });
    ...
    undo: () => {
      ...
      if (prev) {
        c.clear();
        try { c.loadFromJSON(prev, ()=>c.renderAll()); } catch (e) { console.warn(e); }
      }
    },
    redo: () => {
      const c = canvasRef.current; if (!c) return;
      ...
      c.clear();
      try { c.loadFromJSON(next, ()=>c.renderAll()); } catch (e) { console.warn(e); }
    },
    reset: () => {
      const c = canvasRef.current; if (!c) return;
      c.clear(); c.renderAll(); undoStackRef.current = []; redoStackRef.current = []; lastSavedJsonRef.current = null;
    }
  }), []);
  ...
  return (
    <div className="relative w-full h-full bg-gray-50 overflow-hidden" style={{ minHeight: 420 }}>
      <div ref={wrapperRef} style={{ position: 'relative', width: '100%', height: '100%' }}>
        <img ref={baseImgRef} src={mockupUrl || '/placeholder-product.png'} alt="mockup" style={{ position: 'absolute', left: 0, top: 0, width: '100%', height: '100%', objectFit: 'contain', zIndex: 10 }} />
        <div style={{ position: 'absolute', left: (printArea && printArea.left) || 0, top: (printArea && printArea.top) || 0, width: (printArea && printArea.width) || 600, height: (printArea && printArea.height) || 600, border: '2px dashed rgba(0,0,0,0.12)', zIndex: 15, pointerEvents: 'none' }} />
      </div>
    </div>
  );
});
