    ...
    async function load() {
      try {
        ...
        if (res.ok) {
          ...
          else if (data?.ok && Array.isArray(data.products)) list = data.products;
        }
        if (mounted) setProducts(list.filter(p => (p.status || '').toLowerCase() === 'published' || p.active === true));
      } catch (err) {
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 lg:px-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Choose a base product and start designing in our Web-to-Print studio.</p>
          </div>
        </div>
        ...
        <div className="mb-6">
          <nav className="flex gap-2 flex-wrap">
            {TABS.map((t) => (
              <button key={t.id} onClick={() => setActiveTab(t.id)} className={`px-4 py-2 rounded-full text-sm font-medium ${activeTab === t.id ? 'bg-primary text-primary-foreground' : 'bg-border/30 text-foreground'}`}>
                {t.label}
              </button>
            ))}
          </nav>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {visible.map((p) => {
              ...
              const basePrice = p.price || p.basePrice || (p.variants && p.variants[0] && p.variants[0].price) || 0;
              const colors = p.colors || (p.variants || []).map((v:any) => v.color).filter(Boolean) || [];

              return (
                <article key={id} className="group overflow-hidden rounded-lg border border-border bg-card shadow-sm flex flex-col">
                  <div className="relative aspect-4/3 overflow-hidden bg-nude">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center gap-3 bg-primary/70 opacity-0 transition-opacity group-hover:opacity-100">
                      <Button size="sm" onClick={() => navigate({ to: '/configure', search: (q) => ({ ...q, id }) })}>Customize This Product</Button>
                    </div>
                  </div>
                  <div className="p-4 flex-1 flex flex-col">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">From ${Number(basePrice || 0).toFixed(2)}</p>
                      </div>
                    </div>
                    ...
                    <div className="mt-3 flex items-center gap-2">
                      {colors.slice(0,6).map((c:any, idx:number) => {
                        ...
                        return <span key={idx} title={String(hex)} className="w-6 h-6 rounded-full border" style={{ backgroundColor: hex }} />;
                      })}
                      {colors.length > 6 && <span className="text-xs text-muted-foreground">+{colors.length - 6}</span>}
                    </div>
                  </div>
                </article>
              );
