...
function OrderDetailPage() {
  ...
  return (
    <>
      <PageTitle title={order ? `Order ${order.id}` : 'Order Detail'} subtitle="Order information and composite preview" />
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div>
              {order.previewPaths && order.previewPaths[0] ? (
                <img src={order.previewPaths[0]} alt={order.id} className="w-full rounded object-cover" />
              ) : (
            ...
            <div>
              ...
              <pre className="max-h-96 overflow-auto text-xs bg-card p-3 rounded">{JSON.stringify(order.raw, null, 2)}</pre>
              ...
              <div className="mt-4 flex gap-2">
                ...
                <Button variant="outline" onClick={() => navigate({ to: '/admin/orders' })}>Back to Orders</Button>
