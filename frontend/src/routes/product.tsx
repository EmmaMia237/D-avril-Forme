      ...
      <div className="mx-auto max-w-7xl px-4 py-12 lg:px-8">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
          <div className="rounded-lg border border-border bg-card p-4">
            <img src={currentProduct.image} alt={currentProduct.name} className="w-full rounded-md object-cover" />
          </div>
          <div>
            ...
            <p className="mt-4 font-display text-2xl font-semibold text-primary">{formatEur(Number(currentProduct.price || 0))}</p>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <Button
                ...
                onClick={() => {
                  ...
                  try {
                    navigate({ to: "/configure", search: { id: currentProduct._id || currentProduct.id, color: (currentProduct.colors || [])[0] ?? "" } });
                  } catch (e) {
            ...
            <div className="mt-8">
              ...
              <p className="mt-2 text-sm text-muted-foreground">Category: {currentProduct.category}</p>
              <p className="mt-1 text-sm text-muted-foreground">Material: {currentProduct.material}</p>
              <p className="mt-1 text-sm text-muted-foreground">Available colors: {(currentProduct.colors || []).join(", ")}</p>
            </div>
          </div>
        </div>
      </div>
