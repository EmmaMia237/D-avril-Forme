...
function AdminOverview() {
  ...
  async function loadDashboard() {
    try {
      ...
      if (!res.ok) {
        // If the backend indicates missing/invalid token, redirect to login instead of showing raw error
        const errMsg = data?.error || res.statusText || "Unable to load dashboard data";
        if (res.status === 401 && /missing token|invalid token/i.test(String(errMsg))) {
          // navigate to login page for interactive admin to authenticate
                ...
                {loading ? (
                  ...
                  recentOrders.map((order: any) => (
                    <TableRow key={order.id}>
                      ...
                      <TableCell className="text-muted-foreground truncate max-w-[18rem]">{Array.isArray(order.items) ? order.items.map(it => (it && it.name) ? `${it.name} x${it.quantity ?? it.qty ?? 1}` : String(it)).join(', ') : order.items}</TableCell>
                      <TableCell>
                        <StatusPill status={order.status} />
                      </TableCell>
                      <TableCell className="text-right font-semibold">${order.total?.toLocaleString() ?? 0}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
